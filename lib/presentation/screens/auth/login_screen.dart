...
//   // 1️⃣ Registration Logic (इथे डेटाबेसमध्ये डेटा सेव्ह होत नाही)
//   Future<void> _executeSecureSignUp() async {
//     if (!_formKey.currentState!.validate()) return;
...
//       // ईमेल व्हेरीफाय होण्याची वाट पाहा (डेटा नंतर सेव्ह होईल)
...
//   // ईमेल व्हेरीफाय झाल्यावरच Firestore मध्ये डेटा स्टोर करणारा टाइमर
...
//         try {
//           // ईमेल व्हेरीफाय झाल्यावर डेटा सेव्ह करा
...
//   // 2️⃣ Standard Credentials Login (Updated logic)
//   Future<void> _executeUnifiedLogin() async {
//     if (!_formKey.currentState!.validate()) return;
...
//         if (!userDoc.exists) {
//           _showTopNotification(context, "Account error. Profile data missing.", isError: true);
//           await FirebaseAuth.instance.signOut();
//           return;
//         }
...
class _LoginScreenState extends State<LoginScreen> with TickerProviderStateMixin {
  ...
  // 1️⃣ Registration Logic - (FIXED & SAFE UNVERIFIED EMAIL HANDLING)
  Future<void> _executeSecureSignUp() async {
    if (!_formKey.currentState!.validate()) return;
    ...
    try {
      ...
      // १. युझरनेम आधीच वापरले आहे का ते तपासा
      final checkUsername = await db.collection('users').where('username', isEqualTo: usernameInput).get();
      if (checkUsername.docs.isNotEmpty) {
        _showTopNotification(context, 'Username already taken.', isError: true);
        setState(() => _isFormLoading = false);
      ...
      // २. ई-मेल आधीच Verified अकाउंटसाठी वापरला आहे का ते Firestore मध्ये तपासा
      ...
      try {
        // नवीन ऑथ अकाऊंट बनवण्याचा प्रयत्न
        credential = await FirebaseAuth.instance.createUserWithEmailAndPassword(
          email: emailInput,
          password: passwordInput,
        );
      } on FirebaseAuthException catch (e) {
        // ३. जर Firebase Auth मध्ये ई-मेल आधीपासून असेल (पण अन-व्हेरीफाईड असेल)
      ...
      // ४. जर ई-मेल Verified नसेल, तर नवीन व्हेरीफिकेशन लिंक पाठवा
      if (credential.user != null && !credential.user!.emailVerified) {
        ...
        // ५. ईमेल व्हेरीफाय झाल्यावरच Firestore मध्ये स्टोअर होईल
        _startEmailVerificationCheck(
          user: credential.user!,
          uid: credential.user!.uid,
          email: emailInput,
  ...
  void _startEmailVerificationCheck({
    ...
    _emailVerificationTimer = Timer.periodic(const Duration(seconds: 3), (timer) async {
      ...
      // 🔥 युझरने क्लिक (Verify) केल्यावरच डेटा Firestore मध्ये सेव्ह होईल
      if (currentUser != null && currentUser.emailVerified) {
        timer.cancel();
