...
class _LoginScreenState extends State<LoginScreen> with TickerProviderStateMixin {
  ...
  // ⚡ Updated to Support Both Mobile Devices & Web
  Future<void> _executePhoneVerification() async {
    final phoneInput = _phoneController.text.trim();
    if (phoneInput.length != 10 || !RegExp(r'^[0-9]+$').hasMatch(phoneInput)) {
      _showTopNotification(context, 'Please enter a valid 10-digit mobile number!', isError: true);
    ...
    try {
      if (!_showOtpVerificationField || _isOtpExpired) {
        ...
        if (kIsWeb) {
          ...
          await FirebaseAuth.instance.verifyPhoneNumber(
            ...
            verificationCompleted: (PhoneAuthCredential credential) async {
              ...
              _handlePostOtpSuccess(userCred, phoneInput);
            },
            verificationFailed: (FirebaseAuthException e) {
              setState(() => _isOtpLoading = false);
              _showTopNotification(context, e.message ?? 'Verification failed.', isError: true);
            },
        ...
        if (kIsWeb) {
          userCred = await _webConfirmationResult!.confirm(_otpController.text.trim());
        } else {
        ...
        _otpTimer?.cancel();
        await _handlePostOtpSuccess(userCred, phoneInput);
      }
    } catch (e) {
      _showTopNotification(context, 'Invalid OTP Code.', isError: true);
    } finally {
  ...
  Future<void> _handlePostOtpSuccess(UserCredential userCred, String phoneInput) async {
    final db = FirebaseFirestore.instance;
