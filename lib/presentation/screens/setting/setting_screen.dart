...
// // //   // ☁️ Cloud Backup Window (Responsive & Crash-Safe)
// // //   void _startBackupProcessWithProgressWindow() async {
// // //     final email = _backupEmailController.text.trim();
// // //     if (email.isEmpty || !email.contains('@')) {
// // //       CustomerDialogs.showTopNotification(context, 'Please enter a valid email address!', isError: true);
...
// // //                       ),
// // //                       const SizedBox(height: 18),
// // //                       const Text('Cloud Syncing Data...', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)),
// // //                       const SizedBox(height: 12),
// // //                       Container(
// // //                         padding: const EdgeInsets.all(12),
// // //                         decoration: BoxDecoration(color: const Color(0xFF070A0F), borderRadius: BorderRadius.circular(10)),
...
// // //   // 🟢 FIXED: Restore Execution Logic (Handles No Backup & Empty State Perfectly)
// // //   Future<void> _executeSmartReplaceRestore() async {
// // //     final currentUser = FirebaseAuth.instance.currentUser;
// // //     if (currentUser == null) return;
...
// // //       // Check if backup document exists and contains data
// // //       if (!backupDoc.exists || backupDoc.data() == null) {
// // //         if (mounted) {
// // //           setState(() => _isRestoring = false);
...
// // //       // Safety Checks for Empty Arrays in Firestore
...
// // //                   // Responsive Buttons (Column on Mobile, Row on Web/Desktop)
// // //                   LayoutBuilder(
...
// //   // ☁️ 100% SAFE CLOUD BACKUP (NO DIALOG POP CRASH / NO BLACK-WHITE SCREEN)
...
// //       // 🛑 NO RECORDS CHECK -> STOP IMMEDIATELY (NO DIALOG, NO CRASH)
...
// //       // Write directly to cloud backup
// //       final backupRef = FirebaseFirestore.instance.collection('cloud_user_backups').doc(currentUser.uid);
// //       await backupRef.set({
// //         'backupEmail': email,
// //         'operatorUid': currentUser.uid,
...
// //               actions: [
// //                 TextButton(
// //                   onPressed: isAuthenticating ? null : () => Navigator.of(dialogCtx, rootNavigator: true).pop(),
// //                   child: const Text('Cancel', style: TextStyle(color: Colors.grey)),
// //                 ),
// //                 ElevatedButton(
// //                   style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF10B981)),
...
//   // ☁️ ALL PLATFORM SAFE CLOUD BACKUP (Android, Web & iOS)
//   Future<void> _startBackupProcessWithProgressWindow() async {
//     final email = _backupEmailController.text.trim();
//     if (email.isEmpty || !email.contains('@')) {
//       CustomerDialogs.showTopNotification(context, 'Please enter a valid email address!', isError: true);
...
//     try {
//       // 1. CHECK LOCAL DATA FIRST (BEFORE DOING ANYTHING)
//       final customersSnap = await FirebaseFirestore.instance
//           .collection('customers')
//           .where('operatorUid', isEqualTo: currentUser.uid)
//           .get();
...
//       // 🔴 NO RECORDS CHECK -> STOP IMMEDIATELY (NO DIALOG, NO SCREEN CRASH)
//       if (totalCustomers == 0 && totalTx == 0 && totalHistory == 0) {
//         if (mounted) {
//           setState(() => _isBackingUp = false);
//           CustomerDialogs.showTopNotification(
...
class _SettingsControlPanelState extends ConsumerState<SettingsControlPanel> {
  ...
  // ☁️ CIRCULAR PROGRESS CLOUD BACKUP (100% CRASH & WHITE/BLACK SCREEN SAFE)
  Future<void> _startBackupProcessWithProgressWindow() async {
    final email = _backupEmailController.text.trim();
    if (email.isEmpty || !email.contains('@')) {
      CustomerDialogs.showTopNotification(context, 'Please enter a valid email address!', isError: true);
    ...
    try {
      // 1️⃣ DIALOG उघडण्यापूर्वीच डेटा चेक करा
      final customersSnap = await FirebaseFirestore.instance
          .collection('customers')
          .where('operatorUid', isEqualTo: currentUser.uid)
          .get();
      ...
      // 🔴 जर डेटा नसेल तर - शून्य डायलॉग, थेट रेड नोटिफिकेशन अलर्ट!
      if (totalCustomers == 0 && totalTx == 0 && totalHistory == 0) {
        if (mounted) {
          setState(() => _isBackingUp = false);
          CustomerDialogs.showTopNotification(
      ...
      // Progress Trackers sathi ValueNotifiers (StatefulBuilder मुळे होणारे crashes थांबवण्यासाठी)
      final ValueNotifier<int> syncedCustomersNotifier = ValueNotifier<int>(0);
      final ValueNotifier<int> syncedTxNotifier = ValueNotifier<int>(0);
      final ValueNotifier<int> syncedHistoryNotifier = ValueNotifier<int>(0);

      // 2️⃣ CIRCULAR PROGRESS DIALOG SHOW KARA
      showDialog(
        context: context,
        barrierDismissible: false,
        builder: (dialogCtx) {
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              ValueListenableBuilder<int>(
                                valueListenable: syncedCustomersNotifier,
                                builder: (context, count, _) => Text('$count / $totalCustomers', style: const TextStyle(color: Color(0xFF38BDF8), fontWeight: FontWeight.bold, fontSize: 13)),
                              ),
                            ],
                          ),
                          const SizedBox(height: 6),
                          Row(
                            ...
                            children: [
                              const Text('Ledger Records:', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 12, fontWeight: FontWeight.bold)),
                              ValueListenableBuilder<int>(
                                valueListenable: syncedTxNotifier,
                                builder: (context, count, _) => Text('$count / $totalTx', style: const TextStyle(color: Color(0xFF10B981), fontWeight: FontWeight.bold, fontSize: 13)),
                              ),
                            ],
                          ),
                          const SizedBox(height: 6),
                          Row(
                            ...
                            children: [
                              const Text('History Records:', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 12, fontWeight: FontWeight.bold)),
                              ValueListenableBuilder<int>(
                                valueListenable: syncedHistoryNotifier,
                                builder: (context, count, _) => Text('$count / $totalHistory', style: const TextStyle(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 13)),
                              ),
                            ],
                          ),
                        ],
      ...
      // Async Counting Animations
      for (int i = 0; i < totalCustomers; i++) {
        await Future.delayed(const Duration(milliseconds: 15));
        syncedCustomersNotifier.value = i + 1;
      }

      for (int i = 0; i < totalTx; i++) {
        await Future.delayed(const Duration(milliseconds: 10));
        syncedTxNotifier.value = i + 1;
      }

      for (int i = 0; i < totalHistory; i++) {
        await Future.delayed(const Duration(milliseconds: 10));
        syncedHistoryNotifier.value = i + 1;
      }
      ...
      // 🔴 SAFELY POP DIALOG (Prevents Black/White Screen Crash)
                            ...
                            child: SizedBox(
                              ...
                              child: ElevatedButton.icon(
                                ...
                                icon: _isBackingUp 
                                    ? const SizedBox(width: 14, height: 14, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
                                    : const Icon(Icons.cloud_upload_rounded, color: Colors.white, size: 18),
                                label: Text(_isBackingUp ? 'Syncing...' : 'Cloud Backup', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 13)),
                              ),
                            ),
                          ),
                          SizedBox(width: isSmallScreen ? 0 : 12, height: isSmallScreen ? 10 : 0),
