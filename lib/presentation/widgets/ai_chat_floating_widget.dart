...
//   String _ownerName = "Shop Owner"; // Database store owner name
...
//     // 🔄 Listen to Real-time Profile Changes from Firestore Database
//     _listenToOwnerProfileChanges();
...
//   // 📡 Real-time Listener (प्रोफाईल नाव चेंज झाल्यावर लगेच नाव ऑटो-अपडेट होईल)
//   void _listenToOwnerProfileChanges() {
//     try {
...
//       }
//     } catch (e) {
//       debugPrint("Error listening to owner profile: $e");
//     }
//   }

//   @override
//   void dispose() {
//     _userSubscription?.cancel(); // Cancel Firestore Listener
...
//     // 🎯 System Prompt with Updated Owner Name
...
import 'dart:async';
import 'dart:convert';
import 'dart:io'; // 🟢 SocketException हाताळण्यासाठी
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
...
class _AiChatFloatingWidgetState extends ConsumerState<AiChatFloatingWidget>
  ...
  static const String _geminiApiKey =
      'AQ.Ab8RN6Kl_syTzPmYzieu73So9eRGXOEnHu96fXxXaYV1exwl6Q';
  ...
  // 🌐 Gemini API Call (With Offline Network Exception Check)
  Future<void> _sendMessage() async {
    final text = _messageController.text.trim();
    if (text.isEmpty || _isLoading) return;
...
""";
    ...
    try {
      ...
      // 🟢 नेटवर्क/इंटरनेट बंद असताना दाखवायचा मेसेज
      if (mounted) {
        setState(() {
          _messages.add(AiChatMessage(
      ...
      if (mounted) {
        // जर एरर नेटवर्कचा असेल तर सोपा मेसेज दाखवा, अन्यथा मूळ एरर दाखवा
        final String displayError = e.toString().contains("ClientException") ||
